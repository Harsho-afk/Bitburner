...
// Imports
//
import { companyMeta } from 'companyMetadata.js';
...
export async function main(ns) {
    ...
    const growScript = 'growStock.js';  // The grow() script
    ...
    function buyPositions(stock) {
        ...
        if (forecast >= stockBuyOver_Long && volatilityPercent <= stockVolatility) {
            if (playerMoney - moneyKeep > ns.stock.getPurchaseCost(stock, minSharePercent, "Long")) {
                ...
                if (boughtFor > 0) {
                    const message = 'Bought ' + Math.round(shares) + ' Long shares of ' + stock + ' for ' + ns.formatNumber(boughtFor, 3);
            ...
            if (shortUnlock) {
                if (forecast <= stockBuyUnder_Short && volatilityPercent <= stockVolatility) {
                    if (playerMoney - moneyKeep > ns.stock.getPurchaseCost(stock, minSharePercent, "Short")) {
                        ...
                        if (boughtFor > 0) {
                            let message = 'Bought ' + Math.round(shares) + ' Short shares of ' + stock + ' for ' + ns.formatNumber(boughtFor, 3);
    ...
    while (true) {
        ...
        ns.print(' Current Stock Worth: ' + ns.formatNumber(currentWorth, 3));
        ns.print('         Total Profit: ' + ns.formatNumber(totalProfit, 3));
        ns.print('         Total Income: ' + ns.formatNumber(totalIncome, 3) + ' ($' + ns.formatNumber(stockIncome, 1) + '/hr)');
        ns.print('         Net Worth: ' + ns.formatNumber(currentWorth + ns.getPlayer().money, 3));
